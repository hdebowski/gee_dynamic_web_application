...
def add_new_data(data, form_data):
    ...
    elif form_data['satellite'] == 'LandCover':
        data[last_layer_index] = {
            "vis_params": {
                'bands': ['Map'],
            },
    ...
    data[last_layer_index]["name"] = f"Warstwa{last_layer_index} {form_data['name']}"
    return data
...
def generate_layer_data(data):
    for warstwa in data:
        collection = ee.ImageCollection(warstwa['source']) if warstwa['source'] in {DATASETS["Landsat8"], DATASETS["Sentinel2"], DATASETS["LandCover"]} else ee.Image(warstwa['source'])

        if 'date' in warstwa:
            collection = collection.filterDate(warstwa['date'][0], warstwa['date'][1])

        if warstwa['source'] in [DATASETS["Landsat8"], DATASETS["Sentinel2"], DATASETS["LandCover"]]:
        ...
        elif warstwa['source'] == DATASETS["LandCover"]:
